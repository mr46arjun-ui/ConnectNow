...
/*
 * Mood-driven pose grammar. Each sticker gets a distinct body-language pose
 * that amplifies its emotion; strong accent signals win, otherwise a
 * per-family rotation guarantees the six variants never repeat a gesture.
 */
const POSE_BY_ACCENT: Partial<Record<CyberAccentModel, CutePose>> = {
  wave: "wave",
  hand: "wave",
  "finger-gun": "point",
  halo: "pray",
...
const POSE_BY_FACE: Partial<Record<CyberFaceModel, CutePose>> = {
  neutral: "rest",
  smile: "open",
  grin: "raised",
  laugh: "open",
...
const POSE_FAMILY_ROTATION: readonly CutePose[] = [
  "open",
  "raised",
  "wave",
  "clap",
...
function poseFor(face: CyberFaceModel, accent: CyberAccentModel, index: number): CutePose {
  const fromAccent = POSE_BY_ACCENT[accent];
  if (fromAccent) return fromAccent;
  const fromFace = POSE_BY_FACE[face];
  if (fromFace) return fromFace;
  return POSE_FAMILY_ROTATION[index % POSE_FAMILY_ROTATION.length];
}
...
const CUTE_FACE_STICKERS: readonly CuteStickerDefinition[] =
  CYBER_EMOTICONS.map((item, index) => {
    ...
    return {
      ...
      pose: poseFor(item.face, item.accent, index),
    };
  });
